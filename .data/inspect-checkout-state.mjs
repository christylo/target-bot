import { chromium } from "playwright";

const profileDir = "/home/brybry34/target-bot/.data/target-profile";

const context = await chromium.launchPersistentContext(profileDir, {
  headless: true,
  viewport: { width: 1440, height: 1200 }
});
const page = context.pages()[0] ?? await context.newPage();

try {
  await page.goto("https://www.target.com/checkout", { waitUntil: "domcontentloaded" });
  await page.waitForTimeout(4000);
  const buttons = await page.getByRole("button").evaluateAll((nodes) =>
    nodes
      .map((node) => ({
        text: (node.textContent ?? "").replace(/\s+/g, " ").trim(),
        aria: node.getAttribute("aria-label") ?? "",
        dataTest: node.getAttribute("data-test") ?? "",
        disabled: node.hasAttribute("disabled") || node.getAttribute("aria-disabled") === "true"
      }))
      .filter((entry) => entry.text || entry.aria || entry.dataTest)
      .slice(0, 80)
  );
  const headings = await page.getByRole("heading").evaluateAll((nodes) =>
    nodes
      .map((node) => (node.textContent ?? "").replace(/\s+/g, " ").trim())
      .filter(Boolean)
      .slice(0, 40)
  );
  const body = (await page.locator("body").innerText().catch(() => "")).replace(/\s+/g, " ").trim().slice(0, 2000);
  console.log(JSON.stringify({ url: page.url(), headings, buttons, body }, null, 2));
} finally {
  await context.close();
}
